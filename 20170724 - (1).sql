...
--sql 99
--【示例】检索每个产品的编号、名称以及该产品的类型编号和名称。
SELECT p.product_id,p.NAME prd_name,p.product_type_id,t.NAME type_name
FROM products p INNER JOIN product_types  t ON p.product_type_id=t.product_type_id;

SELECT *
FROM products p INNER JOIN product_types  t ON p.product_type_id=t.product_type_id;
--【示例】检索员工编号、姓名以及该员工的工资等级
SELECT e.employee_id,e.first_name||e.last_name emp_name,s.salary_grade_id
FROM employees e INNER JOIN  salary_grades s ON e.salary>=s.low_salary AND e.salary<=s.high_salary;

--检索每个产品的编号、名称以及该产品的类型编号和名称
SELECT p.product_id,p.NAME prd_name,product_type_id,t.NAME type_name
FROM products p INNER JOIN product_types t USING(product_type_id);

SELECT p.product_id,p.NAME prd_name,product_type_id,t.NAME type_name
FROM products p JOIN product_types t USING(product_type_id);

--检索顾客编号、顾客名、该顾客购买的产品名、产品的类型名、所购买产品的数量
SELECT customer_id,cst.First_Name||cst.last_name cst_name,
       product_id,prd.NAME prd_name,
       product_type_id,t.NAME type_name,
       pu.quantity
FROM purchases pu INNER JOIN customers cst USING(customer_id)
     INNER JOIN products prd USING(product_id)
     INNER JOIN product_types t USING(product_type_id);
     
--查询产品信息及其类型名称，某些产品暂时还没有类型。
SELECT p.product_id,product_type_id,p.NAME prd_name,p.DESCRIPTION,p.price,t.NAME type_name
...
--查询产品信息及其类型名称，某些类型暂时还没有对应的产品，也要求输出类型信息
SELECT p.product_id,product_type_id,p.NAME prd_name,p.DESCRIPTION,p.price,t.NAME type_name
FROM products p RIGHT OUTER JOIN product_types t USING(product_type_id);

--full outer join
SELECT p.product_id,product_type_id,p.NAME prd_name,p.DESCRIPTION,p.price,t.NAME type_name
FROM products p FULL OUTER JOIN product_types t USING(product_type_id);
